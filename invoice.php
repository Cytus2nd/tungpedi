...
$pdf->MultiCell(0, 6, ":  " . $pesanan['nama'] . " (" . $pesanan['telepon'] . ")\n   " . $pesanan['alamat'], 0, 1);
$pdf->Ln(8);
...
foreach ($detail_pesanan as $item) {
    ...
    $pdf->Cell(35, 8, 'Rp' . number_format($item['harga'], 0, ',', '.'), 0, 0, 'R');
    $pdf->Ln(18);
}
...
$pdf->Cell(35, 6, 'Rp' . number_format($pesanan['total_harga'] + 7700, 0, ',', '.'), 0, 1, 'R');
$pdf->Ln(2);
$pdf->SetX(115);
$pdf->SetFont('Arial', '', 10);
...
$pdf->Cell(35, 6, 'Rp' . number_format($pesanan['total_harga'] + 9700, 0, ',', '.'), 0, 1, 'R');
$pdf->Ln(10);
...
$pdf->Output('I', 'Invoice_tungpedi.pdf');
